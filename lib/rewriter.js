...
function localFile(file, host) {
  /*var val = []
  var currurl = []
  async function getResp() {
    try {
    const resp = axios.get(`https://${file}`)
    if (resp.host === `https://${host}/${file}`){
      val.push[false]
    } else {
      val.push[false]
    }
...
function rewriteUrlLogic(url, p) {
  ...
  if (pxUrl === "/") {
    console.log(`Converted ${pxUrl} to ${pUrl}`)
    pxUrl = pUrl
  }

  if (validateUrl(url)) {
    //if (localFile(pxUrl, pUrl)) {
    //  var outp = `/proxy/${pUrl}/${pxUrl}`
    //} else {
    if (pxUrl.indexOf('/') === 1) {
      var outp = `/proxy${pxUrl}`
    } else {
      var outp = `/proxy/${pxUrl}`
      //}
    }
  } else {
    console.log(`${pxUrl} Is Invalid`)
    if (url.indexOf('/') === 0) {
      var outp = `/proxy/${pUrl}${pxUrl}`
    } else {
      var outp = `/proxy/${pUrl}/${pxUrl}`
