  ...
  try {
    if (content.includes('html')) {
      ...
      if (content.includes('javascript')) {
        var node = acorn.parse(html, { ecmaVersion: 2020 })
        
        return html.toString().replace(/(,| |=|\()document.location(,| |=|\)|\.)/gi, str => { return str.replace('.location', `.dlocation`); }).replace(/(,| |=|\()window.location(,| |=|\)|\.)/gi, str => { return str.replace('.location', `.dlocation`); })
      } else {
        return html
      }
    }
